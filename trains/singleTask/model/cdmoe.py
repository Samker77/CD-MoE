...
class CDMOE(nn.Module):
    def __init__(self, args):
        ...
        self.use_new_router = getattr(args, 'use_new_router', False)
        if self.use_new_router:
            router_input_dim = self.orig_d_l + self.orig_d_a + self.orig_d_v
        else:
            # 原始方式：每个模态的特征维度乘以文本长度（50），再求和
            router_input_dim = (self.orig_d_l * self.len_l +
                                self.orig_d_a * self.len_l +
                                self.orig_d_v * self.len_l)
    ...
    def forward(self, text, audio, video):
        ...
        # m_w = self.Router(m_i)
        feat_l = text  # [batch, seq_len, orig_d_l]
        feat_a = audio  # [batch, seq_len, orig_d_a]
        feat_v = video  # [batch, seq_len, orig_d_v]
        if self.use_new_router:
            # 新方法：各模态时序均值拼接
            feat_l_mean = feat_l.mean(dim=1)  # [batch, orig_d_l]
            feat_a_mean = feat_a.mean(dim=1)  # [batch, orig_d_a]
            feat_v_mean = feat_v.mean(dim=1)  # [batch, orig_d_v]
            router_input = torch.cat([feat_l_mean, feat_a_mean, feat_v_mean], dim=-1)  # [batch, total_dim]
        else:
            # 原始方法：拼接对齐后的序列特征（保持原逻辑）
            if not self.aligned:
                audio_ = self.transfer_a_ali(audio.permute(0, 2, 1)).permute(0, 2, 1)
                video_ = self.transfer_v_ali(video.permute(0, 2, 1)).permute(0, 2, 1)
                m_i = torch.cat((text, video_, audio_), dim=2)
                ...
                m_i = torch.cat((text, video, audio), dim=2)
            # 原始代码中 m_i 形状为 [batch, seq_len, total_dim]，router 期望二维输入，因此展平
            router_input = m_i.view(m_i.size(0), -1)  # [batch, seq_len * total_dim]
...
#         # ========== 修改 Router 输入维度 ==========
#         # 新输入维度：orig_d_l + orig_d_a + orig_d_v（各模态特征的均值拼接）
#         router_input_dim = self.orig_d_l + self.orig_d_a + self.orig_d_v
#         self.Router = router(router_input_dim, 3, self.args.temperature)
#         # ==========================================
...
#         # ========== 修改 Router 输入为各模态均值拼接 ==========
#         # 计算各模态的时间维度均值 [batch, orig_d]
#         feat_l_mean = feat_l.mean(dim=1)  # [batch, orig_d_l]
#         feat_a_mean = feat_a.mean(dim=1)  # [batch, orig_d_a]
#         feat_v_mean = feat_v.mean(dim=1)  # [batch, orig_d_v]
        
#         # 拼接均值特征 [batch, orig_d_l + orig_d_a + orig_d_v]
#         router_input = torch.cat([feat_l_mean, feat_a_mean, feat_v_mean], dim=-1)
...
#         if self.fusion_method == "sum": 
#             for i in range(m_w.shape[0]):
#                 c_f = c_l_att[i] * m_w[i][0] + c_a_att[i] * m_w[i][1] + c_v_att[i] * m_w[i][2]
#                 if i == 0: c_fusion = c_f.unsqueeze(0)
#                 else: c_fusion = torch.cat([c_fusion, c_f.unsqueeze(0)], dim=0)   
#         elif self.fusion_method == "concat":        
#             for i in range(m_w.shape[0]):
