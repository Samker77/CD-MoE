...
from trains.singleTask.model import cdmoe
import sys
...
def CDMOE_run(
    ...
    if model_save_dir == "":
        model_save_dir = Path.home() / "CDMOE" / "saved_models"
    Path(model_save_dir).mkdir(parents=True, exist_ok=True)
    if res_save_dir == "":
        res_save_dir = Path.home() / "CDMOE" / "results"
    Path(res_save_dir).mkdir(parents=True, exist_ok=True)
    if log_dir == "":
        log_dir = Path.home() / "CDMOE" / "logs"
...
def _run(args, num_workers=4, is_tune=False, from_sena=False):
    ...
    model = getattr(cdmoe, 'CDMOE')(args).cuda()
    ...
    if args.mode == 'test':
        ...
        model.load_state_dict(torch.load('pt/cdmoe.pth'))
